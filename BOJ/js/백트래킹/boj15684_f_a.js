...
// 입력 파싱
// n: 세로선 개수, m: 기존 가로선 개수, h: 가로선을 놓을 수 있는 위치 수
...
for (let i = 1; i <= m; i++) {
  ...
  // ✨ 방향을 바로 알 수 있다. -> 좌 우 인덱스 범위 확인 & 해당 위치 연결되어있는지 과정 생략 가능
  map[a][b] = RIGHT;
  map[a][b + 1] = LEFT;
}
...
/**
 * 백트래킹 함수
 * @param {number} y 현재 탐색할 시작 행
 * @param {number} x 현재 탐색할 시작 열
 * @param {number} addedCnt 현재까지 추가한 가로선 수
 * @param {number} finishCnt 목표로 하는 가로선 수
 */
